...
print(data.frame(ByLibSize=sum(libsize.drop), ByFeature=sum(feature.drop), ByMito=sum(mito.drop), Remaining=ncol(sce)))
cat("  \n")
print(droppedcells)
```
...
```{r regress_confounding, echo=FALSE,warning=FALSE,message=FALSE}
...
# so_temp <- MeanVarPlot(so_temp ,fxn.x = expMean, fxn.y = logVarDivMean, x.low.cutoff = 0.0125, x.high.cutoff = 3, y.cutoff = 0.5, do.contour = F)
...
### **MeanVar Plot**
```{r Seurat_meanvar, echo=FALSE,warning=FALSE,message=FALSE}
so <- MeanVarPlot(so ,fxn.x = expMean, fxn.y = logVarDivMean, x.low.cutoff = 0.0125, x.high.cutoff = 3, y.cutoff = 0.5, do.contour = F)
```

### **PCAs**

```{r Seurat_pcas, echo=FALSE,warning=FALSE,message=FALSE,results='asis'}
so <- PCA(so, pc.genes = so@var.genes, do.print = TRUE, pcs.print = 5, genes.print = 5)
so <- ProjectPCA(so,do.print=F)
```
