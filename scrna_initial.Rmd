...
so <- CreateSeuratObject(so.data, min.genes = 0, min.cells=round(0.001*ncol(sce)), project=projectId)
# so@raw.data <- so@data
...
if(grepl("hg",species)|grepl("mm",species)){
  ...
  legend("topright", paste0("[Unknown] ",length(which(assignments$score$G1 >= 0.5 & assignments$score$G2M >= 0.5))), bty="n", text.col="red")
...
if(grepl("hg",species)|grepl("mm",species)){
  ...
  scores <- gsva(exprs(eset_filtered),gsets,min.sz=5,max.sz=500,verbose=TRUE)
